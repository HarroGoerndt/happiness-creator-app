...
def is_abo_active(user_pin):
    return user_pin in ["FAMILIE123", "TESTZUGANG", "HARRO1"]
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS chat (user_id TEXT, topic TEXT, subtopic TEXT, message TEXT, response TEXT, summary TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS community (user_id TEXT, content TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS marketplace (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT, title TEXT, description TEXT, price TEXT, kontakt_email TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, sender_id TEXT, receiver_id TEXT, item_id INTEGER, item_title TEXT, message TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS dating (user_id TEXT PRIMARY KEY, name TEXT, interesse TEXT, beschreibung TEXT, image_path TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    conn.commit()
    conn.close()
...
Du beginnst jede Unterhaltung als sokratischer Dialog.

Deine Fragen kommen nicht aus Belehrung, sondern aus Wahrnehmung.

Du stellst klare, einfache Fragen, aber nicht viele - und nur, wenn sie wirklich nötig sind.

Du sprichst weniger als du zuhörst.

Du spiegelst Gedanken und fasst sanft zusammen, statt zu analysieren.

Du gibst Raum, statt ihn zu füllen.

In jeder Antwort zeigst du, dass du wirklich zuhörst.
Du wiederholst nie Phrasen wie „Ich verstehe“ - sondern zeigst Verstehen durch Sprache, Klarheit und Haltung.

Sprich wie ein vertrauter Mensch am Kaffeetisch - nahbar, freundlich, leicht, aber tief.

Du redest nur, wenn du weißt, warum. Du gibst keine Tipps, keine Lösungen, keine Bewertungen.
Du stellst in Frage, lädst ein, öffnest.

Am Ende jeder Antwort gibst du einen einzigen klaren Satz zum Mitnehmen -
etwas, das klingt wie ein leiser Gedanke, der bleibt.

❌ Keine neuen Fragen.
❌ Keine Aufgaben.
❌ Kein Coaching.
❌ Kein Weiterreden. sei nie eine ki, kein bot.

Du bleibst einfach da, klar, respektvoll, wach
Thema: {topic}  
Unterthema: {subtopic}
"""
    ...
    with st.expander("📦 Neues Angebot erstellen"):
        ...
        if st.button("Angebot veröffentlichen"):
            if title and description and price:
                c.execute("INSERT INTO marketplace (user_id, title, description, price, kontakt_email) VALUES (?, ?, ?, ?, ?)", (st.session_state.user_id, title, description, price, kontakt_email))
                conn.commit()
            ...
            with st.expander("📝 Kontakt aufnehmen"):
                if is_abo_active(st.session_state.get("user_pin", "")):
                    ...
                    if st.button("Nachricht senden", key=f"send_{item_id}"):
                        c.execute("INSERT INTO messages (sender_id, receiver_id, item_id, item_title, message) VALUES (?, ?, ?, ?, ?)", (st.session_state.user_id, seller_id, item_id, title, message))
                        conn.commit()
...
def messages_tab():
    ...
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT message, timestamp, sender_id, item_title FROM messages WHERE receiver_id = ? ORDER BY timestamp DESC", (st.session_state.user_id,))
    messages = c.fetchall()
    if not messages:
        st.info("Keine Nachrichten erhalten.")
        conn.close()
        return
    for msg, ts, sender_id, artikel in messages:
        st.markdown(f"""
📦 **Artikel**: *{artikel}*  
...
def dating_tab():
    ...
    with st.form("dating_form"):
        ...
        if st.form_submit_button("Eintragen") and name.strip() and interesse.strip():
            image_path = None
            if bild and bild.name:
                ...
                image_path = filename
            try:
                c.execute("INSERT OR REPLACE INTO dating (user_id, name, interesse, beschreibung, image_path) VALUES (?, ?, ?, ?, ?)", (st.session_state.user_id, name, interesse, beschreibung, image_path))
                conn.commit()
    ...
    try:
        c.execute("SELECT name, interesse, beschreibung, timestamp, image_path FROM dating WHERE user_id != ? ORDER BY timestamp DESC LIMIT 20", (st.session_state.user_id,))
        results = c.fetchall()
    except Exception as e:
    ...
    if not results:
        ...
        for name, interesse, beschreibung, ts, img in results:
            ...
            st.markdown(f"🧑 **{name}** – sucht: *{interesse}*  \n🕒 {ts}  \n💬 {beschreibung}")
            st.markdown("---")
    conn.close()
